from ingestion.load_multiple_controles import load_all_controles
from processing.normalization import normalize_columns
from processing.anonymization import anonimizar_cpf
from cleaning.clean_data import clean_dataset, clean_users_dataset, clean_first_access_dataset, clean_first_access_llm_dataset, clean_open_ticket_dataset, clean_history_ticket_dataset

#def build_dataset():
#    df_original = load_all_controles()
#    df = normalize_columns(df_original)
#
#    return df

def build_db_dataset(df):
    df = normalize_columns(df)
    df_final = clean_first_access_dataset(df)

    return df_final

def build_db_first_access_dataset(df):
    df = normalize_columns(df)
    df_final = clean_first_access_dataset(df)

    return df_final

def build_db_open_ticket_dataset(df):
    df = normalize_columns(df)
    df_final = clean_open_ticket_dataset(df)

    return df_final

def build_db_history_ticket_dataset(df):
    df = normalize_columns(df)
    df_final = clean_history_ticket_dataset(df)

    return df_final

def build_dataset_anonymized(dataset):
    df = dataset

    df["cpf_hash"] = df["cpf/cnpj"].apply(anonimizar_cpf)
    df = df.drop(columns=["cpf/cnpj"])

    #Para limpar o dataset antes de retornar o final
    df_final = clean_dataset(df)

    return df_final

def build_db_dataset_anonymized(dataset):
    df = dataset

    df["cpf_hash"] = df["usuario"].apply(anonimizar_cpf)
    df = df.drop(columns=["usuario"])

    #Para limpar o dataset antes de retornar o final
    df_final = clean_users_dataset(df)

    return df_final

def build_db_dataset_for_llm(dataset):
    df = dataset

    #Para limpar o dataset antes de retornar o final
    df_final = clean_first_access_llm_dataset(df)

    return df_final